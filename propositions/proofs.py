...
class InferenceRule:
    ...
    def variables(self) -> Set[str]:
        ...
        # Task 4.1
        # Joan
        variables = self.conclusion.variables()
    ...
    def specialize(self, specialization_map: SpecializationMap) -> \
        ...
        new_assumptions = tuple(a.substitute_variables(specialization_map) for a in self.assumptions)
    ...
    def _formula_specialization_map(general: Formula, specialization: Formula) \
        ...
        if is_variable(general.root):
            spemap[general.root] = specialization
            return spemap   
        elif general.root == specialization.root:
            if is_constant(general.root):
                return spemap
            elif is_unary(general.root):
                first = InferenceRule._formula_specialization_map(general.first, specialization.first)
                if first != None:
                    spemap = InferenceRule._merge_specialization_maps(spemap, first)
                    return spemap
                else:
                    return None
            elif is_binary(general.root):
                first = InferenceRule._formula_specialization_map(general.first, specialization.first)
                second = InferenceRule._formula_specialization_map(general.second, specialization.second)
                if first != None and second != None:
                    spemap = InferenceRule._merge_specialization_maps(spemap, first)
                    spemap = InferenceRule._merge_specialization_maps(spemap, second)
                    return spemap
                else:
                    return None
        else:
    ...
    def specialization_map(self, specialization: InferenceRule) -> \
        ...
        if len(self.assumptions) != len(specialization.assumptions):
            return None
        spemap = InferenceRule._formula_specialization_map(self.conclusion, specialization.conclusion)
        if self.assumptions and specialization.assumptions:
            for (a, asp) in zip(self.assumptions, specialization.assumptions):
                amap = InferenceRule._formula_specialization_map(a, asp)
                spemap = InferenceRule._merge_specialization_maps(spemap, amap)
        return spemap        
        # Joan
...
class Proof:
    ...
    def rule_for_line(self, line_number: int) -> Union[InferenceRule, None]:
        ...
        if Proof.Line.is_assumption(line):
            ...
            assumptions = tuple(self.lines[previousline].formula for previousline in line.assumptions)
    ...
    def is_line_valid(self, line_number: int) -> bool:
        ...
        if Proof.Line.is_assumption(line):
            ...
            if line.rule not in self.rules:
                ...
                c1 = self.rule_for_line(line_number).is_specialization_of(line.rule) # rule_for_line returns the concrete InferenceRule that we want to prove. We need to check that it is a specialization of the abstract rule allowed
                c2 = all(previousline < line_number for previousline in line.assumptions)
                c3 = all(source == assumption for source, assumption in zip([self.lines[a].formula for a in line.assumptions], self.rule_for_line(line_number).assumptions))
                if c1 and c2 and c3:
                    return True
                else:
                    return False
    ...
    def is_valid(self) -> bool:
        ...
        # Task 4.6c
        # Joan
        if self.lines: # Lines cannot be empty! Even if only one line with the conclusion itself
            c1 = self.statement.conclusion == self.lines[-1].formula
            c2 = all( self.is_line_valid(i) for i in range(len(self.lines)) )
            if c1 and c2:
                return True
        else:
            return False
...
def prove_specialization(proof: Proof, specialization: InferenceRule) -> Proof:
    ...
    # Task 5.1
    # Joan
    # We only need to map the lines in the proof. The rules can remain general.
    spemap = proof.statement.specialization_map(specialization)
    lines = list()
    for L in proof.lines:
        formula = L.formula.substitute_variables(spemap)
        rule = getattr(L, 'rule', None)
        assumptions =  getattr(L, 'assumptions', None)
        lines.append(Proof.Line(formula, rule, assumptions))
    lines = tuple(lines)
...
def _inline_proof_once(main_proof: Proof, line_number: int,
    ...
    lines = list() 
    for L in main_proof.lines[:line_number]:
        lines.append(L)

    speproof = prove_specialization(lemma_proof, Proof.rule_for_line(main_proof, line_number))
    for L in speproof.lines:
        if not Proof.Line.is_assumption(L):
            assumptions = (a + line_number for a in L.assumptions)
            lines.append(Proof.Line(L.formula, L.rule, assumptions))
        else:
            if L.formula not in main_proof.statement.assumptions:
                for earlier_line in main_proof.lines:
                    if L.formula == earlier_line.formula:
                        rule, assumptions = earlier_line.rule, earlier_line.assumptions
                        break
                lines.append(Proof.Line(L.formula, rule, assumptions))
            else:
                lines.append(L)
    ...
    for L in main_proof.lines[line_number+1:]:
        ...
        if L.is_assumption():
            ...
            assumptions = (a + len(lemma_proof.lines)-1 if a >= line_number else a for a in L.assumptions)
        lines.append(Proof.Line(formula, rule, assumptions))
    ...
    while cond == True:
        for line_number, line in enumerate(proof.lines):
            if line.rule == lemma_proof.statement:
                ...
                break
            elif line_number == len(proof.lines) - 1:
                cond = False

    rules = main_proof.rules | lemma_proof.rules
    rules = rules - set([lemma_proof.statement])
    
    return Proof(main_proof.statement, rules, proof.lines)
    # Joan
