...
def prove_and_commutativity() -> Proof:
    ...
    rules = frozenset({A_RULE, AE1_RULE, AE2_RULE})
    lines = list()
    lines.append(Proof.Line(Formula.parse('(p&q)'), None, None))
    lines.append(Proof.Line(Formula.parse('p'), AE2_RULE, (0,)))
    lines.append(Proof.Line(Formula.parse('q'), AE1_RULE, (0,)))
    lines.append(Proof.Line(Formula.parse('(q&p)'), A_RULE, (2,1)))
    lines = tuple(lines)
...
def prove_I0() -> Proof:
    ...
    # Joan
    statement = I0
    rules = frozenset({MP, I1, D})
    lines = list()
    lines.append(Proof.Line(Formula.parse('((p->((p->p)->p))->((p->(p->p))->(p->p)))'), D, () ))
    lines.append(Proof.Line(Formula.parse('(p->(p->p))'), I1, () ))
    lines.append(Proof.Line(Formula.parse('(p->((p->p)->p))'), I1, () ))
    lines.append(Proof.Line(Formula.parse('((p->(p->p))->(p->p))'), MP, (2,0) ))
    lines.append(Proof.Line(Formula.parse('(p->p)'), MP, (1,3) ))
    lines = tuple(lines)
    return Proof(statement, rules, lines)
    # Joan
...
def prove_hypothetical_syllogism() -> Proof:
    ...
    # Task 5.5
    # Joan
    easier_statement = InferenceRule([Formula.parse('(p->q)'), Formula.parse('(q->r)'), Formula.parse('p')], Formula.parse('r'))
    rules = frozenset({MP, I0, I1, D})
    
    lines = list()
    lines.append(Proof.Line(Formula.parse('(p->q)'), None, None))
    lines.append(Proof.Line(Formula.parse('(q->r)'), None, None))
    lines.append(Proof.Line(Formula.parse('p'), None, None))
    lines.append(Proof.Line(Formula.parse('q'), MP, (2,0) ))
    lines.append(Proof.Line(Formula.parse('r'), MP, (3,1) ))
    lines = tuple(lines)
    
    easier_proof = Proof(easier_statement, rules, lines)
    
    return remove_assumption(easier_proof)
    # Joan
